...
@router.get("/institutions", response=List[InstitutionOut])
def list_institutions(request):
    """Returns a list of all supported financial institutions."""
    return FinancialInstitution.objects.all()

@router.get("/institutions/{institution_id}", response=InstitutionOut)
...
def create_product(request, payload: FinancialProductIn):
    ...
    # Verify owner belongs to the same family if provided
    owner = None
    if payload.owner_id:
